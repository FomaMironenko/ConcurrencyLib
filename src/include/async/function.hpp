...
template <class F> friend inline auto make_async(ThreadPool& pool, F&& fun);
...
inline AsyncResult<Ret> call_async(ThreadPool& pool, Fun&& fun, Args &&...args)
{
    ...
    FunctionType<Ret, void> task = std::bind(
        std::forward<Fun>(fun),
        std::forward<Args>(args)...
    );
    ThreadPool::Task pool_task = std::make_unique<details::AsyncTask<Ret> >(
        std::move(task),
        std::move(promise)
    );
...
template <class Fun>
template <class Ret, class ...Args>
AsyncResult<Ret> AsyncFunction<Fun>::invoke(Args &&...args)
{
    return call_async<Ret>(
        *parent_pool_,
        callable_,
        std::forward<Args>(args)...
    );
...
template <class Fun>
template <class ...Args>
AsyncResult<std::invoke_result_t<Fun, Args...> > AsyncFunction<Fun>::operator()(Args &&...args)
{
    return call_async<std::invoke_result_t<Fun, Args...> >(
        *parent_pool_,
        callable_,
        std::forward<Args>(args)...
    );
